...
export default function MangaViewRating({
  ...
  function AverageStar({ position, value }: { position: number; value: number }) {
    ...
    const color = full || half ? maggaColors.archiveGold : "rgba(255,255,255,0.25)";
    if (full) return <StarIcon sx={{ fontSize: 20, color }} />;
    if (half) return <StarHalfIcon sx={{ fontSize: 20, color: maggaColors.archiveGold }} />;
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", gap: 0.5 }}>
      ...
      {!hideAverage && <Box sx={{ display: "flex", alignItems: "center", gap: 0.25 }}>
        ...
        <Typography variant="body2" sx={{ color: maggaColors.archiveGold, fontWeight: 700, ml: 0.75 }}>
      ...
      {!hideInteractive && <Box sx={{ display: "flex", alignItems: "center", gap: 0.25 }}>
          ...
          <Tooltip key={star} title={`ให้ ${star} ดาว`} placement="top">
            <span>
              <IconButton
                ...
                sx={{ p: 0.25, color: displayRating >= star ? maggaColors.archiveGold : "rgba(255,255,255,0.3)", transition: "color 0.15s" }}
              >
        ...
        {isSubmitting && <CircularProgress size={14} sx={{ ml: 0.5, color: maggaColors.archiveGold }} />}
